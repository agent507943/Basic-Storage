...
def play_sound(kind="correct"):
    """Play a short feedback sound with DHCP-themed tones. Uses winsound on Windows when available, otherwise pygame mixer if present."""
    if not settings.get("sound_enabled", True):
        return
    try:
        if USE_WINSOUND:
            if kind == "correct":
                # Network success tone - ascending notes
                winsound.Beep(500, 100)
                winsound.Beep(700, 150)
            elif kind == "wrong":
                # Network error tone - descending buzz
                winsound.Beep(400, 200)
                winsound.Beep(250, 250)
            elif kind == "finish":
                # DHCP lease success - three ascending tones
...
class DHCPLearnApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Learn DHCP - Dynamic Host Configuration Protocol")
    ...
    def _build_study_tab(self):
        lbl = ttk.Label(self.study_frame, text="Study: DHCP - Dynamic Host Configuration Protocol", font=(None, 16))
        lbl.pack(pady=8)
